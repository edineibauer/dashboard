...
if(!empty($_SESSION['userlogin']['setor'])) {
    ...
    if(empty($dados['dashboard'])) {
        ...
        //GENÉRICO
        if(empty($dados['dashboard'])) {
            if (file_exists(PATH_HOME . "dash/0/panel.php")) {
                ob_start();
                include_once PATH_HOME . "dash/0/panel.php";
                $dados['dashboard'] = ob_end_flush();
            }
            ...
            if(empty($dados['dashboard'])) {
                foreach (\Helpers\Helper::listFolder(PATH_HOME . VENDOR) as $lib) {
                    if (file_exists(PATH_HOME . VENDOR . "{$lib}/dash/0/panel.php")) {
                        ob_start();
                        include_once PATH_HOME . VENDOR . "{$lib}/dash/0/panel.php";
                        $dados['dashboard'] = ob_end_flush();
                        break;
                    }
                }
